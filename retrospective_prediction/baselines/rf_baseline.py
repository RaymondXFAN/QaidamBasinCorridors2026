...
# -*- coding: utf-8 -*-
"""
Random Forest baseline for settlement prediction.
...
class RandomForestBaseline:
    """Random Forest baseline conforming to unified BaseModel interface."""
    ...
    def __init__(self, config: dict):
        ...
        # RF hyperparameters
        self.n_estimators = int(self.config.get('n_estimators', 100))
    ...
    def _flatten_X(self, X: np.ndarray) -> np.ndarray:
        ...
        if X.ndim == 3:
            N, T, F = X.shape
            print(f"[Random Forest] Flattening input: ({N}, {T}, {F}) → ({N}, {T*F})")
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        print(f"[Random Forest] Training with n_estimators={self.n_estimators}, "
              f"max_depth={self.max_depth}")
        print(f"[Random Forest] Input shape: {X_flat.shape}, Target shape: {y_flat.shape}")
